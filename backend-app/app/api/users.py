...
def update_user_state(new_state:bool,
...
def read_users(current_user: Annotated[api_m.User, Depends(get_current_active_admin)],
               skip: int = 0,
               limit: int = 100,
               db: Session = Depends(get_db),
               ):
...
def read_user_by_mail(user_mail: str,
                      current_user: Annotated[api_m.User, Depends(get_current_active_admin)],
                      db: Session = Depends(get_db)):
...
def update_any_user_state(user_id: int,
                      new_state:bool,
                      current_user: Annotated[api_m.User, Depends(get_current_active_admin)],
                      db: Session = Depends(get_db)):
    db_user = crud.get_user(db, user_id=user_id)
